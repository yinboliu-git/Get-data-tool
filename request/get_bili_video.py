...
def send_request(url_1,headers):
...
def merge_data(video_data, name):
    """数据的合并"""

    video_name = video_data['title']
    print('视频合成开始:', video_data['title'])
    # ffmpeg -i video.mp4 -i audio.wav -c:v copy -c:a aac -strict experimental output.mp4
    COMMAND = f'ffmpeg -i {video_name}.mp4 -i {video_name}.mp3 -c:v copy -c:a aac -strict experimental {name}.mp4'
...
def get_video(url):
    ...
    merge_data(video_data, name)
    shutil.move('./'+name, path_name+name)
    shutil.move('./'+video_data['title']+'.mp3', path_name+video_data['title']+'.mp3')
    shutil.move('./'+video_data['title']+'.mp4', path_name+video_data['title']+'.mp4')

get_url('https://www.bilibili.com/video/BV17b411q7EV')
